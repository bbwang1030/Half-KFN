...
for i in tqdm(range(len(X_te))):
    ...
    y_te_i = np.array([y_te[i]])
    x_te_i = np.array([np.reshape(X_te[i], orig_dims)])
    # print(type(y_te_i))
    # print(len(np.reshape(X_te[i], orig_dims)))
    adv_sample = attack(x_te_i, labels=y_te_i, max_epsilon=max_epsilon)
